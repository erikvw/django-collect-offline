...
from django_collect_offline.transaction import (
...
def my_test_parser(json_text):
    model = "collect_offline_app.testmodeldates"
    field = "f2"
...
class TestDeserializer2(TestCase):
    ...
    def test_deserialize_with_m2m(self):
        ...
        m2m = M2m.objects.using("client").create(name="erik", display_name="Erik")
        obj.m2m.add(m2m)
        m2m = M2m.objects.using("client").create(name="bob", display_name="Bob")
        obj.m2m.add(m2m)
