...
class TestSync(TestCase):
    ...
    def setUp(self):
        ...
        offline_models = ['django_offline.testmodel',
                          'django_offline.badtestmodel',
                          'django_offline.anotherbadtestmodel',
                          'django_offline.yetanotherbadtestmodel',
                          'django_offline.testmodelwithfkprotected',
                          'django_offline.testmodelwithm2m',
                          'django_offline.testofflinemodelnohistorymanager',
                          'django_offline.testofflinemodelnouuid']
    ...
    def test_raises_on_no_historical_manager(self):
        with override_settings(DEVICE_ID='10'):
            try:
                TestOfflineModelNoHistoryManager.objects.using(
                    'client').create()
            ...
            with self.assertRaises(OutgoingTransaction.DoesNotExist):
                try:
                    OutgoingTransaction.objects.using('client').get(
                        tx_pk=test_model.pk,
                        tx_name='django_offline.testmodel',
                        action=INSERT)
            ...
            with self.assertRaises(OutgoingTransaction.DoesNotExist):
                try:
                    OutgoingTransaction.objects.using('client').get(
                        ...
                        tx_name='django_offline.historicaltestmodel',
                        action=INSERT)
                except OutgoingTransaction.DoesNotExist:
                    pass
                else:
                ...
                try:
                    outgoing.update(
                        test_model_with_fk=OutgoingTransaction.objects.using('client').get(
                            ...
                            tx_name='django_offline.testmodelwithfkprotected',
                            action=INSERT))
                except OutgoingTransaction.DoesNotExist:
                    pass
                else:
                ...
                try:
                    outgoing.update(
                        test_model_with_fk_historical=(
                            OutgoingTransaction.objects.using('client').get(
                                tx_pk=history_obj.history_id,
                                tx_name='django_offline.historicaltestmodelwithfkprotected',
                                action=INSERT)))
                except OutgoingTransaction.DoesNotExist:
                    pass
                else:
    ...
    def test_created_obj_serializes_to_correct_db(self):
        ...
        self.assertListEqual(
            result,
            ['django_offline.historicaltestmodel', 'django_offline.testmodel'])
        self.assertListEqual(
    ...
    def test_updated_obj_serializes_to_correct_db(self):
        ...
        result.sort()
        self.assertListEqual(
            result, ['django_offline.historicaltestmodel',
                     'django_offline.testmodel'])
        self.assertListEqual(
        ...
        self.assertListEqual(
            result,
            ['django_offline.historicaltestmodel',
             'django_offline.historicaltestmodel',
             'django_offline.testmodel'])
